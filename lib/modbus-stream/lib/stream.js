...
function Stream(transport, options) {
	...
	if (options.debug !== null) {
		this.debugger = function (mode, data) {
			...
			if (options.debugdate === false) {
				options.debug.info('ModbusStream', util.format("%s 0x[ %s ]\n", mode, bytes.map(function (byte) { return padStart(byte.toString(16).toUpperCase(), 2, "0"); }).join(", ") ));
			} else {
				options.debug.info('ModbusStream', util.format("%s %s 0x[ %s ]\n", (new Date).toISOString(), mode, bytes.map(function (byte) { return padStart(byte.toString(16).toUpperCase(), 2, "0"); }).join(", ") ));
			}
		};
	...
	this.transport.on("incoming-data", function (data) {
		stream.emit('incoming-data');
	});
	this.transport.on("outgoing-data", function (data) {
		stream.emit('outgoing-data');
	});
